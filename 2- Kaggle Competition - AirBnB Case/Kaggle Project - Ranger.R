...
baseData$price[is.na(baseData$price)] <- 1
...
##Step 6
modelRanger = ranger(price~minimum_nights+number_of_reviews + bedrooms+ 
                       accommodates + security_deposit + cleaning_fee + bed_type+
                       guests_included + host_response_rate + room_type + 
                       property_type_upd+ review_scores_cleanliness +
                       review_scores_rating + amenities+
                       neighbourhood_group_cleansed +  
                       calculated_host_listings_count_private_rooms , data=train, num.trees = 1000)

summary(modelRanger)


predTrain <- predict(modelRanger, data=train, num.trees = 1000)
caret::postResample(pred = predTrain$predictions, train$price)


##Step 7
predTest <- predict(modelRanger, data=test, num.trees = 1000)
caret::postResample(pred = predTest$predictions, test$price)


##Step 8
predScore = predict(modelRanger, data=score, num.trees = 1000)
caret::postResample(pred = predScore$predictions, score$price)

submissionFile = data.frame(id = score$id, price = predScore$predictions)
write.csv(submissionFile, './MayaBayram_FinalPredictions.csv',row.names = F)
...
modelForest = randomForest(price~minimum_nights+number_of_reviews+room_type+ property_type_upd + bedrooms +  review_scores_cleanliness +
                             ...
                             guests_included + host_response_rate,
                           data=train,
                           n.trees = 5000)
summary(modelForest)
